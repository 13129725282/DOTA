...
#  DOTA数据的txt文件夹
txtFolder = r"G:\DOTA\org\train\labelTxt"
#  DOTA数据的image文件夹
imgFolder = r"G:\DOTA\org\train\images"
#  要复制到的image文件夹
copy_imageFolder = r"G:\DOTA\train\object\baseball-diamond\images"
#  要复制到的txt文件夹
copy_txtFolder = r"G:\DOTA\org\train\object\baseball-diamond\labels"
#  感兴趣类别
selected_class = "baseball-diamond"
sum = 0
    ...
    if(os.path.splitext(txtNameList[i])[1] == ".txt"):
        ...
        while line:
            ...
            if(selected_class in line):
                ...
                sum+=selected_class_num
                break
            #  若第一行不是selected_class，继续向下读，直到读取完文件
            else:
                line = f.readline() 
print(selected_class,"共有标签:",sum)
f.close() #关闭文件
